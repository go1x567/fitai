// === Screen: My Outfit ===
const OutfitScreen = ({ outfit, onRemove, onContinue, onTryOn }) => {
  const items = outfit.map(id => CATALOG.find(c => c.id === id)).filter(Boolean);
  const total = items.reduce((s, i) => s + i.price, 0);

  // build full outfit colors
  const fullOutfit = React.useMemo(() => {
      ...
      <div className="outfit-header" style={{ display: 'flex', alignItems: 'end', justifyContent: 'space-between', marginBottom: 32, gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-ghost">
            <ShareIcon size={15} /> Поделиться образом
          </button>
...
window.OutfitScreen = OutfitScreen;
